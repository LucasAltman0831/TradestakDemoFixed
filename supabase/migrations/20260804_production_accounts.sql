...
delete from public.supplier_profiles where source_name='TradeStak Seed' or external_id like 'seed-%';
...
begin
  ...
  company:=coalesce(nullif(trim(new.raw_user_meta_data->>'company_name'),''),'New TradeStak Company');
...
insert into public.supplier_profiles(owner_id,name,slug,claimed,verified,is_public,source_name,external_id)
select p.id,coalesce(nullif(trim(p.company_name),''),'New TradeStak Company'),trim(both '-' from regexp_replace(lower(coalesce(nullif(trim(p.company_name),''),'new-tradestak-company')),'[^a-z0-9]+','-','g'))||'-'||left(replace(p.id::text,'-',''),8),true,false,true,'Supplier signup',p.id::text
