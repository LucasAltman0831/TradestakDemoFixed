...
function SupplierCard({supplier,saved,onSave,onPreview,selected,onCompare}:{supplier:Supplier;saved:boolean;onSave:()=>void;onPreview:()=>void;selected:boolean;onCompare:()=>void}){
  ...
  return <article className={`${styles.supplierCard} ${expanded?styles.expandedCard:''}`}>
    ...
    {expanded&&<div className={styles.expandedPreview}><p>{supplier.description}</p><div>{supplier.certifications.map(c=><span key={c}><Check size={10}/>{c}</span>)}</div></div>}
    <footer><label><input type="checkbox" checked={selected} onChange={onCompare}/>Compare</label><button onClick={()=>setExpanded(v=>!v)}>{expanded?'Close preview':'Quick preview'} <ChevronDown size={13}/></button><button className={styles.profileAction} onClick={onPreview}>View profile <ArrowRight size={13}/></button></footer>
  </article>;
}
  ...
  return <div className={styles.marketplace}>
    ...
    <main id="market">
      ...
      <div className={styles.content}>
        ...
        <div className={styles.results}><div className={styles.resultsHeader}><div><span>SUPPLIER DISCOVERY</span><h2>{filtered.length} trusted partners</h2><p>Showing suppliers across North Alabama</p></div><div><button className={styles.mobileFilter} onClick={()=>setFiltersOpen(v=>!v)}><SlidersHorizontal size={14}/>Filters</button><select aria-label="Sort suppliers"><option>Best match</option><option>Highest score</option><option>Most reviewed</option><option>Most experience</option></select></div></div>
