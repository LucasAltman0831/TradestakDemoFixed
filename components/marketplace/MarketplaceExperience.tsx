...
const suppliers:Supplier[]=[
  {id:1,slug:'northline-concrete',name:'Northline Concrete Co.',initials:'NC',trade:'Concrete',location:'Athens, Alabama',score:92,quality:95,delivery:89,communication:93,value:90,years:15,service:'North Alabama',projects:184,reviews:38,verified:false,accent:'#c98728',projectType:'Residential',description:'Residential and light-commercial concrete partner known for dependable dispatch, consistent mix quality, and proactive scheduling.',certifications:['NRMCA Certified','DBE','OSHA Compliant']},
...
function SupplierCard({supplier,saved,onSave,onPreview,selected,onCompare}:{supplier:Supplier;saved:boolean;onSave:()=>void;onPreview:()=>void;selected:boolean;onCompare:()=>void}){
  ...
  return <article className={`${styles.supplierCard} ${expanded?styles.expandedCard:''}`}>
    ...
    <footer><label><input type="checkbox" checked={selected} onChange={onCompare}/>Compare</label><button onClick={()=>setExpanded(v=>!v)}>{expanded?'Close preview':'Quick preview'} <ChevronDown size={13}/></button>{supplier.verified?<button className={styles.profileAction} onClick={onPreview}>View profile <ArrowRight size={13}/></button>:<Link className={styles.profileAction} href={`/claim/${supplier.slug}`}>Claim Your Profile <ArrowRight size={13}/></Link>}</footer>
  </article>;
}
