...
function PlanAction({role,plan}:{role:Role;plan:Plan}){if(role==='builders'&&plan.name==='Pro')return <CheckoutButton endpoint="/api/stripe/checkout/builder" label="Start Business Pro" className={styles.planButton}/>;if(role==='suppliers'&&plan.name==='Growth')return <CheckoutButton endpoint="/api/stripe/checkout/supplier-verified" label="Start Supplier Growth" className={styles.planButton}/>;if(role==='suppliers'&&plan.name==='Premium')return <CheckoutButton endpoint="/api/stripe/checkout/supplier-premium" label="Start Supplier Premium" className={styles.planButton}/>;if(plan.name==='Enterprise')return <Link className={styles.planButton} href="/signup?role=builder">Request enterprise access <ArrowRight size={14}/></Link>;return <Link className={styles.planButton} href={role==='builders'?'/signup?role=builder':'/signup?role=supplier'}>{role==='builders'?'Get started':'Claim Your Profile'} <ArrowRight size={14}/></Link>}

const plans:Record<Role,Plan[]>={
  builders:[
    {name:'Free',monthly:0,yearly:0,purpose:'Explore the supplier network and start building your shortlist.',features:['Browse the live supplier marketplace','View public scores and reviews','Save up to 5 suppliers','Submit verified evaluations','3 company or capability images']},
    {name:'Pro',monthly:299,yearly:239,purpose:'Give your procurement team measurable tools for managing supplier risk.',recommended:true,eyebrow:'RECOMMENDED FOR PROCUREMENT TEAMS',features:['Unlimited saved suppliers','Live network health analytics','Supplier risk flags','Downloadable network CSV','25 company or engagement images','Everything in Business Free']},
    {name:'Enterprise',monthly:null,yearly:null,purpose:'Plan a tailored rollout for larger procurement organizations.',features:['Everything in Business Pro','Custom rollout planning','Security and data review','Commercial terms for your organization','Direct implementation consultation']},
  ],
  suppliers:[
    {name:'Claim Free',monthly:0,yearly:0,purpose:'Own your company presence and keep your essential information current.',features:['Claim and manage your company profile','Public marketplace visibility','Current reputation scores','Manage service details','4 company or capability images']},
