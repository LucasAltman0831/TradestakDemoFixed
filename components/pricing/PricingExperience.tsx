...
export function PricingExperience(){
  ...
  return <div className={styles.page}>
    ...
    <main>
      ...
      <section className={styles.plansSection} id="plans"><header><div><span>{role==='builders'?'BUILDERS & BUYERS':'SUPPLIERS & TRADE PARTNERS'}</span><h2>{role==='builders'?'Reduce supplier risk before it impacts your project.':'Turn your reputation into new opportunities.'}</h2></div><p>{role==='builders'?'Move from supplier discovery to confident project decisions with verified performance intelligence.':'Start with your profile, then unlock the tools that turn strong performance into growth.'}</p></header><div className={styles.planGrid}>{activePlans.map((plan,index)=><article className={`${styles.planCard} ${plan.recommended?styles.recommended:''}`} key={plan.name}>{plan.recommended&&<div className={styles.recommendedLabel}><Zap size={11}/> {plan.eyebrow}</div>}<div className={styles.planTop}><span>0{index+1}</span><h3>{plan.name}</h3><p>{plan.purpose}</p></div><div className={styles.price}>{plan.monthly===null?<strong>Custom</strong>:<><strong>${annual?plan.yearly:plan.monthly}</strong><span>/month</span></>} {annual&&plan.monthly!==null&&plan.monthly>0&&<small>Billed annually</small>}</div><PlanAction role={role} plan={plan}/><div className={styles.features}><b>WHAT&apos;S INCLUDED</b>{plan.features.map(feature=><span key={feature}><Check size={13}/>{feature}</span>)}</div></article>)}</div><div className={styles.assurance}><span><CircleCheckBig size={16}/>No setup fees</span><span><CircleCheckBig size={16}/>Cancel anytime</span><span><CircleCheckBig size={16}/>Scores are never pay-to-play</span><a href="#compare">Compare every feature <ArrowRight size={13}/></a></div></section>
