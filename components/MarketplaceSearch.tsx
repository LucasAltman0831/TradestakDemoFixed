...
export function MarketplaceSearch({suppliers}:{suppliers:Supplier[]}){
  ...
  function toggleCompare(supplier:Supplier){
    setCompare(current=>current.some(item=>item.id===supplier.id)?current.filter(item=>item.id!==supplier.id):current.length<comparisonLimit?[...current,supplier]:current);
  }
  ...
  return <>
    ...
    <div className="resultsSummary"><p><b>{rows.length}</b> supplier{rows.length===1?'':'s'} found</p><span>Numeric scores appear after 3 eligible verified business evaluations.</span></div>
