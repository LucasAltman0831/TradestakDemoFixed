...
export default async function TrustReviewPage(){
  ...
  const [{data:verifications},{data:reviews}]=await Promise.all([
    ...
    supabase.from('reviews').select('id,builder_user_id,supplier_profile_id,project_name,project_type,body,created_at,evaluations(relationship_context)').eq('moderation_status','pending').order('created_at'),
  ]);
  const builderIds=[...new Set((reviews??[]).map((item:any)=>item.builder_user_id))];
  ...
  return <main className="shell"><Sidebar role="admin"/><TrustReviewExperience verifications={(verifications??[]).map((item:any)=>({...item,profileCompany:requesterMap.get(item.requester_user_id)||null}))} reviews={(reviews??[]).map((item:any)=>({...item,reviewerCompany:builderMap.get(item.builder_user_id)||'Verified builder',supplierName:supplierMap.get(item.supplier_profile_id)||'Supplier',relationshipContext:item.evaluations?.relationship_context||null}))}/></main>;
}
