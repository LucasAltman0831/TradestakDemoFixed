...
export default async function Page({params}:{params:Promise<{slug:string}>}){
  ...
  const [{data:reviews},{data:media}]=await Promise.all([
    supabase.from('reviews').select('id,project_name,project_type,body,verified,created_at').eq('supplier_profile_id',supplier.id).eq('is_public',true).order('created_at',{ascending:false}).limit(10),
  ...
  return <>
    ...
    <main className={styles.page}>
      <section className={styles.hero}>
        <div className={styles.identity}><span>{supplier.name.split(' ').map((part:string)=>part[0]).slice(0,2).join('')}</span><div><p>{supplier.trade_category||'Construction supplier'} · {location}</p><h1>{supplier.name}</h1><div>{supplier.verified?<SupplierVerificationBadge/>:supplier.claimed?<span className="badge">Claimed · verification pending</span>:<UnclaimedProfileBadge/>}</div></div></div>
        <TradeStakScoreBadge score={supplier.score??0} label={hasScores?'Based on builder evaluations':'Not yet rated'}/>
      </section>
      <section className={styles.layout}>
        <div>
          <article className={styles.about}><span>COMPANY OVERVIEW</span><h2>Company information</h2><p>{supplier.description||'This company has not added a description yet.'}</p><div className={styles.facts}><div><MapPin/><span>Location<b>{location}</b></span></div><div><Globe2/><span>Service area<b>{supplier.service_area||'Not provided'}</b></span></div><div><Building2/><span>Profile status<b>{supplier.verified?'TradeStak verified':supplier.claimed?'Claimed':'Unclaimed'}</b></span></div></div></article>
