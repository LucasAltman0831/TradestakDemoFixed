...
export function planLabel(role:'builder'|'supplier',subscription:SubscriptionRow){if(!hasPaidAccess(subscription))return role==='builder'?'Builder Free':'Supplier Free';if(subscription?.plan==='builder_pro')return 'Builder Pro';if(subscription?.plan==='supplier_premium')return 'Supplier Premium';return 'Supplier Growth';}
